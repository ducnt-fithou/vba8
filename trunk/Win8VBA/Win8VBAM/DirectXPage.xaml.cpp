...
#else
		return assetsFolder->GetFileAsync("Pong.gb");
#endif
...
void DirectXPage::EnableButtons(bool enable)
{
	...
	this->CheatButton->IsEnabled = enable;
}
...
void Win8Snes9x::DirectXPage::CheatButton_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
	if(this->loadingDialogOpen)
		return;

	Popup ^popup = ref new Popup();
	popup->Width = this->windowBounds.Width - 200;
	popup->Height = this->windowBounds.Height - 200;
	popup->IsLightDismissEnabled = false;

	CheatPane ^pane = ref new CheatPane();	
	pane->Width = this->windowBounds.Width - 200;
	pane->Height = this->windowBounds.Height - 200;
	popup->Child = pane;

	popup->Opened += ref new EventHandler<Object ^>([this](Object ^sender, Object ^args)
	{
		this->emulator->Pause();
	});
	popup->Closed += ref new EventHandler<Object ^>([this, pane](Object ^sender, Object ^args)
	{
		if(pane->Cancelled)
		{
			this->emulator->Unpause();
